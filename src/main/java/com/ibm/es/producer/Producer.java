...
public class Producer {
	...
	private static ResourceBundle producerTranslations = ResourceBundle.getBundle("MessageBundle", Locale.getDefault());
	private static ResourceBundle configTranslations = ResourceBundle.getBundle("ConfigBundle", Locale.getDefault());
	...
	private static final String DEFUALT_PAYLOAD_DELIMITER = "\\n";
	...
	public static void main(String[] args) {
		...
		try {
			...
			producer.topic = res.getString("topic");
			producer.numRecords = res.getLong("numRecords");
			producer.recordSize = res.getInt("recordSize");
			producer.throughput = res.getInt("throughput");
			producer.configFilePath = res.getString("producerConfigFile");
			producer.payloadFilePath = res.getString("payloadFile");
			producer.shouldPrintMetrics = res.getBoolean("printMetrics");
			producer.numThreads = res.getInt("numThreads");
			producer.size = res.getString("size");

			producer.payloadDelimiter = res.getString("payloadDelimiter").equals("\\n") ? "\n"
			...
			if(res.getBoolean("genConfig")) {
				try {
					StringBuilder template = new StringBuilder(IOUtils.resourceToString("/producer.config.template", null));
				...
				if(producer.getTopic() == null || 
				   ...
				   (producer.getRecordSize() == null && producer.getPayloadFilePath() == null)) {
					...
					for (int i = 0; i < producer.numThreads; i++) {
						ProducerThread producerThread = new ProducerThread(producers, String.format("producer%d", i), producer);
	...
	private static ArgumentParser argParser() {
		ArgumentParser parser = ArgumentParsers.newFor("es-producer")
				.singleMetavar(true)
				.build()
				.defaultHelp(true)
				.description(producerTranslations.getString("producer.summary"));
		
		ArgumentGroup propertiesFileSection = parser.addArgumentGroup(producerTranslations.getString("producer.configSection"));
		
		propertiesFileSection.addArgument("-g", "--gen-config")
		...
		ArgumentGroup requiredParams = parser.addArgumentGroup(producerTranslations.getString("producer.requiredConfigSection"));
		
		requiredParams.addArgument("-t", "--topic")
		...
		requiredParams.addArgument("-c", "--producer-config")
				.action(Arguments.store())
				.required(false)
				.type(String.class)
				.metavar("CONFIG-FILE")
		...
		ArgumentGroup generalConfig = parser.addArgumentGroup(producerTranslations.getString("producer.generalConfigSection"));

		generalConfig.addArgument("-s", "--size")
				.action(Arguments.store())
				.required(false)
				.type(String.class)
				.metavar("SIZE")
		...
		generalConfig.addArgument("-n", "--num-records")
				.action(Arguments.store())
				.required(false)
				.type(Long.class)
		...
		generalConfig.addArgument("-T", "--throughput")
				.action(Arguments.store())
				.required(false)
				.type(Integer.class)
				.metavar("THROUGHPUT")
		...
		generalConfig.addArgument("-d", "--payload-delimiter")
				.action(Arguments.store())
				.required(false)
				.type(String.class)
				.metavar("PAYLOAD-DELIMITER")
				.dest("payloadDelimiter")
				.setDefault(DEFUALT_PAYLOAD_DELIMITER)
				.help(producerTranslations.getString("producer.payloadDelimeter.help"));

		generalConfig.addArgument("-m", "--print-metrics")
				.action(Arguments.storeTrue())
				.required(false)
				.type(Arguments.booleanType())
				.dest("printMetrics")
		...
		generalConfig.addArgument("-x", "--num-threads")
				.action(Arguments.store())
				.required(false)
				.type(Integer.class)
				.metavar("NUM_THREADS")
		...
		payloadOptions.addArgument("-r", "--record-size")
				.action(Arguments.store())
				.required(false)
				.type(Integer.class)
				.metavar("RECORD-SIZE")
		...
		payloadOptions.addArgument("-f", "--payload-file")
				.action(Arguments.store())
				.required(false)
				.type(String.class)
	...
	private static void overrideArgumentsWithEnvVars(Producer producer) {
		...
		if (env.containsKey("ES_TOPIC")) producer.topic = env.get("ES_TOPIC");
		if (env.containsKey("ES_RECORD_SIZE")) producer.recordSize =  Integer.parseInt(env.get("ES_RECORD_SIZE"));
		if (env.containsKey("ES_NUM_THREADS")) producer.numThreads =  Integer.parseInt(env.get("ES_NUM_THREADS"));
		if (env.containsKey("ES_PRODUCER_CONFIG")) producer.configFilePath = env.get("ES_PRODUCER_CONFIG");
		if (env.containsKey("ES_PAYLOAD_FILE")) producer.payloadFilePath = env.get("ES_PAYLOAD_FILE");
		if (env.containsKey("ES_PAYLOAD_DELIMITER")) producer.payloadDelimiter = env.get("ES_PAYLOAD_DELIMITER");

		if (env.containsKey("ES_THROUGHPUT")) {
			producer.throughput = Integer.parseInt(env.get("ES_THROUGHPUT"));
			throughputOverridden = true;
		}
		if (env.containsKey("ES_NUM_RECORDS")) {
			producer.numRecords = Long.parseLong(env.get("ES_NUM_RECORDS"));
			numRecordsOverridden = true;
		}
		if (throughputOverridden && numRecordsOverridden) {
			producer.size = "";
		}
		if ((throughputOverridden && !numRecordsOverridden) || (!throughputOverridden && numRecordsOverridden)) {
